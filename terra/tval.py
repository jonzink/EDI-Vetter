...
try:
	...
	from terra import FFA

	import terra.keptoy
	import terra.tfind
	import terra.keplerio
	import terra.config
	import terra.utils.hdfstore
...
def add_phasefold(lc, t, P, t0, plM, starting_phase=-0.5):
    ...
    lc['t_phasefold_m{}'.format(plM)] = t_phasefold
    lc['phase_m{}'.format(plM)] = phase
    lc['cycle_m{}'.format(plM)] = cycle
    return lc
...
try:
	...
	class DataValidation(terra.utils.hdfstore.HDFStore):
	    """ Object for handling transit validation checks
...
def label_transit(t, P, t0, tdur, cpad=0.5, cfrac=3):
    ...
    labels  = np.zeros(t.size, dtype=list(zip( names, [int] * len(names) )) )
    labels['transit_id'] -= 1
...
def local_detrending(lc, P, t0, tdur, poly_degree=3, min_continuum=2, 
    ...
    fldt = list(map(detrend,[ lcdt.ix[idx] for idx in list(g.groups.values()) ] ))
    fldt = pd.concat(fldt)
    lcdt['f'] = fldt
    return lcdt
...
class TransitModel(object):
    ...
    def __init__(self, P, t0, rp, apl, b, u1=0.44, u2=0.66, ecc=0.0, w=0.0, limb_dark='quadratic',
                  subtract_one=False, batman_kw=None):
        if batman_kw==None:
            batman_kw = {}
        ...
        lm_params.add('b', value=b, vary=True, min=0.0)
        lm_params.add('apl', value=apl, vary=True, min=0.0)  
        # lm_params.add('u1', value=u1, vary=False)
        # lm_params.add('u2', value=u2, vary=False)
...
class CosModel(object):
    ...
    def __init__(self, amp, offSet, YoffSet, freq):
        sin_params = Parameters()
        sin_params.add('amp', value=amp, vary=True)
        sin_params.add('offSet', value=offSet, vary=True)
        sin_params.add('YoffSet', value=offSet, vary=True)
        sin_params.add('freq', value=freq, vary=True)
        self.sin_params = sin_params


    def model(self, sin_params, t):
        from numpy import exp, sign, cos, pi
        amp = sin_params['amp'].value
        offSet = sin_params['offSet'].value
        YoffSet = sin_params['YoffSet'].value
        freq = sin_params['freq'].value
        
        _model =amp*cos(freq*t+offSet)+YoffSet
        return _model

    def residual(self, sin_params, t, f, ferr):
        """Normalized residual"""
        _model = self.model(sin_params, t)
        resid = ((f-_model) / ferr)
        return resid

class FPModel1(object):
...
class FPModel2(object):

    def __init__(self, offSet,dVal,aVal,t0):
        sin_params = Parameters()
        sin_params.add('offSet', value=offSet, vary=True )
        sin_params.add('dVal', value=dVal, vary=True )
        sin_params.add('aVal', value=aVal, vary=True )
        sin_params.add('t0', value=t0, vary=True )
        self.sin_params = sin_params


    def model(self, sin_params, t):
        offSet = sin_params['offSet'].value
        dVal= sin_params['dVal'].value
        aVal=sin_params['aVal'].value
        t0=sin_params['t0'].value
        _model =offSet+dVal/(1+np.exp(-aVal*(t-t0)))
        return _model
...
class FPModel3(object):

    def __init__(self, offSet,dVal, aVal,t0,beta):
        sin_params = Parameters()
        sin_params.add('offSet', value=offSet, vary=True )
        sin_params.add('dVal', value=dVal, vary=True )
        sin_params.add('aVal', value=aVal, vary=True )
        sin_params.add('t0', value=t0, vary=True )
        sin_params.add('beta', value=beta, vary=True )
        self.sin_params = sin_params
    ...
    def model(self, sin_params, t):
        ...
        aVal=sin_params['aVal'].value
        t0=sin_params['t0'].value
        beta=sin_params['beta'].value
        _model =offSet+dVal/(1+np.exp(-aVal*(t-t0)))*(1-np.exp(beta*t))
        return _model
...
class FPModel4(object):

    def __init__(self, offSet, dVal, aVal,t0, tau):
        sin_params = Parameters()
        sin_params.add('offSet', value=offSet, vary=True )
        sin_params.add('dVal', value=dVal, vary=True )
        sin_params.add('aVal', value=aVal, vary=True )
        sin_params.add('t0', value=t0, vary=True )
        sin_params.add('tau', value=tau, vary=True )
        self.sin_params = sin_params
    ...
    def model(self, sin_params, t):
        ...
        aVal=sin_params['aVal'].value
        t0=sin_params['t0'].value
        tau=sin_params['tau'].value
        _model =offSet+dVal/(1+np.exp(-aVal*((t-tau/2)-t0)))-dVal/(1+np.exp(-aVal*((t+tau/2)-t0)))
        return _model
...
def phase_fold(t, fm, P, t0, tdur, cfrac=3, cpad=1, LDT_deg=1, nCont=4):
    ...
    dtype = list(zip(['tPF','f','t'],[float]*3))
    cut   = ~tm.mask
    lcPF  = np.array( list(zip(tm[cut],fldt[cut],t[cut])) ,  dtype=dtype )
...
try:
	...
	def checkHarm(dM,P,tbase,harmL0=terra.config.harmL,ver=True):
	    """
	    Evaluate S/N for (sub)/harmonics
	    """
	    MESL  = []
